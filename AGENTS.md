...
- **No Borders on Standalone Text**: Never add borders, border boxes, or boxed pill outlines around standalone text elements (such as employee numbers, reference codes, staff IDs, metadata values, or inline labels). Keep standalone text crisp, clean, and unbordered, distinguishing it using typography, font weights, color tokens (such as `text-portal-accent` or `text-portal-muted`), and `font-mono` rather than enclosing it in artificial borders or boxes.
- **Button Variants & Dark Theme Text Contrast**: Button variants (`FlatButton`, `buttonVariants.ts`) must always use high-contrast text and theme tokens appropriate for the dark portal background. Specifically, `outline` and `ghost` variants must use crisp white or high-readability text (`!text-white` or `!text-portal-text`), 1px portal borders (`!border-portal-border hover:!border-portal-border/80`), and subtle translucent hover states (`hover:!bg-white/[0.08]`). Never use dark text colors (such as `text-slate-800` or `text-slate-700`) on buttons, as they become invisible against dark portal cards, modals, and canvas backgrounds.
- **Data Table Cell Typography & Font Size Hierarchy**: Never use font sizes in data table rows, headers, or column cells that are larger than the portal sidebar navigation items (which use compact `text-xs` / `12px`). Primary cell values (such as staff names, user names, branch names, or item titles) must use `text-xs font-bold` (or `font-semibold`), while secondary metadata (such as emails, phone numbers, or subtitles) must use `text-[11px]`. Table cell typography must feel sleek, dense, and proportional to the portal's compact control aesthetic, never oversized or bulky.
- **Immediate Data Table Refresh (`resetTableData`) on Entity Mutations**: Whenever any entity or record is created, updated, deleted, invited, granted access, or has its status modified across any form, action, or modal in the application (e.g. adding staff, inviting users, editing staff, granting platform access, managing roles, suspending/activating accounts), the executing handler or modal **MUST immediately call `resetTableData()`** (imported from `components/data-table`) as soon as the mutating API call succeeds. Never delay the table refresh until a modal or reveal dialog is manually dismissed by the user. Invoking `resetTableData()` immediately dispatches the global `tableRefreshEvent`, ensuring the underlying data table refetches and displays the latest records instantly.
